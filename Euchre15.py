# Lead Strategies Class
# Play Strateges Class
...
class LivePlayer(Player):
        ...
        if bidding_round==0:
            ...
            if bid_type > 0:
                ...
                while trump not in validtrump:
                    try: trump=input("Which suit? (hearts=0, spades=1, diamonds=2, clubs=3)")
                    except: trump=999
    ...
    def play(self,leadsuit_local,trump_local):
        ...
        while pc not in legit:
            try: pc = input("Which card? (1 through "+str(len(self.hand))+"): ")
            except: pc = 999
            if pc in legit:
                if tricksequence.index(self) > 0:
...
while not realplayer:
    try: realplayer = input("Do you want to be a player? 1=yes, 2= no): ")
    except: realplayer==0
    if realplayer=="1":
...
while not games:
    try: games = int(input("How many games?"))
    except: games=0
...
while game < games:
    ...
    for player in Players:
        if lpactive and player==0: player.showhand(trump,0)
