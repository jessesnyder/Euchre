            ...
            for card in self.hand:
                if card == LB_local:
                    if not card == self.hand[0]:
                        print(","),  # ,end=''
                    print(
                        "Jack of " + suitlabels[card[0]] + " [left bauer]"
                    ),  # ,end=""
    ...
    class LivePlayer(Player):
        ...
        def play(self, leadsuit_local, trump_local):
            ...
            while pc not in legit:
                try:
                    pc = int(
                        input("Which card? (1 through " + str(len(self.hand)) + "): ")
                    )
                except ValueError:
                    pc = 999
                if pc in legit:
                    if tricksequence.index(self) > 0:
                        if (
                            len(self.getsuit(leadsuit_local, self.hand, trump_local))
                            > 0
                        ):
                            if (
                                self.hand[pc - 1] == LB_local
                                and leadsuit == trump_local
                            ):
                                pass
                            elif self.hand[pc - 1][0] != leadsuit_local and (
                                pc in legit
                            ):
